...
用户描述题目需求
  ...
  → 如 PASS：
      → 如有平台凭据：docker push → 注册镜像 → 导入题目 → 自动化交付
      → 如无凭据：交付文件包 + 手动操作说明
```
...
## Platform Import（API 自动导入，v2）

当用户配置了平台凭据时，reviewer 通过后可自动将题目导入平台。

### 前置条件

设置环境变量或配置文件 `~/.gzctf/config.json`：

```bash
export GZCTF_HOST=platform.example.com
export GZCTF_TOKEN=gzctf_pat_xxxx.yyyyyyyy
```

Token 需要以下 scope：`challenges:read`, `challenges:write`, `challenges:delete`, `operations:read`, `images:read`, `images:write`, `images:delete`，以及对应比赛资源授权（如 `game:123`）。

See `prompts/_api.md` for full API integration details.
...
Reviewer PASS
  → 如用户提供了 gameId + 凭据：
      → 方案 A（内网通 10.24.0.28:5000）：
          docker tag + docker push → python3 scripts/ctf_client.py image register-reference
      → 方案 B（离线或外部）：
          docker save → python3 scripts/ctf_client.py image upload-archive
      → 轮询 image wait-ready
      → 生成 challenge JSON → python3 scripts/ctf_client.py challenge import
      → 轮询完成，输出 challengeId
      → 写入 import-result.json 到题目目录
  → 如用户未提供 gameId：
      → 输出手动操作步骤（当前流程）
```
...
| 条件 | 方案 | 命令 |
|------|------|------|
| `10.24.0.28:5000` 可达 | A: Registry 引用 | `docker tag` + `docker push` + `register-reference` |
| 不可达 | B: Archive 上传 | `docker save` + `upload-archive` |
